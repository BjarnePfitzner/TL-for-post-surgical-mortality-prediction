...
def train_model(model, loss_object,
                ...
                train_size, class_weights, cfg, log_file, log_to_wandb):

    metrics_logger = MetricsLogger(log_file, log_to_wandb)
    max_steps = math.inf
    if cfg.dp.type == 'LDP':
    ...
    for round in range(cfg.training.total_rounds):
        ...
        if round % 5 == 0:
            ...
            metrics_logger.log({key: val for key, val in result_dict.items()}, step=round)
    ...
    result_dict.update({'test/calibrated_f1': calibrated_f1.numpy(),
                        ...
                        'test/calibrated_f2_threshold': f2_threshold.numpy(),
                        'test/loss': test_loss.result().numpy()})
    metrics_logger.log({key: val for key, val in result_dict.items()}, step=(round + 1))
    ...
    pretty_print_results_dict(result_dict, (round + 1))
    metrics_logger.write_metrics_to_file()

    if early_stopping_round is None:
        early_stopping_round = cfg.training.total_rounds
    result_dict['early_stopping_round'] = early_stopping_round

    return model, result_dict, {'labels': labels, 'pred_probs': pred_probs, 'pred_labels': pred_labels}
